...
- (void)drawRect:(CGRect)rect {
    ...
    if (self.type == HMSegmentedControlTypeText) {
        [self.sectionTitles enumerateObjectsUsingBlock:^(id titleString, NSUInteger idx, BOOL *stop) {
            ...
            NSRange range = [titleString rangeOfString:@"我接收的("];
            ...
            if (self.selectedSegmentIndex == idx) {
                ...
                if (range.location != NSNotFound && ![[titleString substringWithRange:NSMakeRange(range.length, [(NSString *)titleString length] - range.length - 1)] isEqualToString:@"0"]) {
                    
                    NSDictionary * dic1 = @{NSFontAttributeName:[UIFont boldSystemFontOfSize:self.font.pointSize], NSForegroundColorAttributeName:self.selectedTextColor};
                    NSDictionary * dic2 = @{NSFontAttributeName:[UIFont boldSystemFontOfSize:self.font.pointSize], NSForegroundColorAttributeName:[UIColor redColor]};
                    
                    NSMutableAttributedString * string = [[NSMutableAttributedString alloc] initWithString:titleString];
                    [string addAttributes:dic1 range:NSMakeRange(0, [(NSString *)titleString length])];
                    [string addAttributes:dic2 range:NSMakeRange(range.length, [(NSString *)titleString length] - range.length - 1)];
                ...
                if (range.location != NSNotFound && ![[titleString substringWithRange:NSMakeRange(range.length, [(NSString *)titleString length] - range.length - 1)] isEqualToString:@"0"]) {
                    ...
                    NSDictionary * dic1 = @{NSFontAttributeName:[UIFont boldSystemFontOfSize:self.font.pointSize], NSForegroundColorAttributeName:self.textColor};
                    NSDictionary * dic2 = @{NSFontAttributeName:[UIFont boldSystemFontOfSize:self.font.pointSize], NSForegroundColorAttributeName:[UIColor redColor]};
