...
//wjj add 
- (instancetype) initWithMissionInputTitle:(NSString *)title
                             detail:(NSString *)detail
                        placeholder:(NSString *)inputPlaceholder
                            handler:(MMPopupInputHandler)inputHandler{
    ...
    NSArray *items =@[
                      MMItemMake(config.defaultTextCancel, MMItemTypeHighlight, nil),
                      MMItemMake(@"审核并上报", MMItemTypeHighlight, nil),
                      MMItemMake(@"审核通过", MMItemTypeHighlight, nil)
                      ];
    return [self initWithTitle:title detail:detail items:items inputPlaceholder:inputPlaceholder inputHandler:inputHandler];
...
- (instancetype) initWithOldTextInputTitle:(NSString *)title
                                    detail:(NSString *)detail
                               placeholder:(NSString *)inputPlaceholder
...
- (instancetype) initWithBackInputTitle:(NSString *)title
                                    detail:(NSString *)detail
                               placeholder:(NSString *)inputPlaceholder
                                   oldText:(NSString *)oldText
                                   handler:(MMPopupInputHandler)inputHandler{
...
//            self.inputView.placeholder = inputPlaceholder;
            self.inputView.keyboardType=_boardType;
            self.inputView.text=_oldText?_oldText:@"";
        ...
        for ( NSInteger i = 0 ; i < items.count; ++i )
        {
            ...
            [btn mas_makeConstraints:^(MASConstraintMaker *make) {
                ...
                else
                {
                    ...
                    {
                        firstButton = btn;
                        make.left.equalTo(self.buttonView.mas_left).offset(0);
                    }
                    else
                    {
...
//                    }
        ...
        [lastButton mas_updateConstraints:^(MASConstraintMaker *make) {
            ...
            {
                 make.right.equalTo(self.buttonView.mas_right).offset(0);
...
- (void)actionButton:(UIButton*)btn
{
    ...
    }else if (self.withKeyboard && (btn.tag==2)){
        if ( self.inputView.text.length > 0 )
        {
            [self hide];
        }
...
- (instancetype)init
{
    ...
    if ( self )
    {
        ...
        self.itemHighlightColor = COMMON_BLUE_COLOR;//MMHexColor(0xE76153FF);
