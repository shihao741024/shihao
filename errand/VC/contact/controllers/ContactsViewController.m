...
@interface ContactsViewController ()<SRRefreshDelegate,UITableViewDelegate,UITableViewDataSource>
...
- (void)RecordsearchClick
{
    ...
    searchCtrl.feedBackSearchDataBlock = ^(NSString *keywords,NSString *kind,NSString *department,NSString *hospitalrank,NSString *province,NSString *city) {
        
        NSArray *searchArray = @[keywords,kind,department,hospitalrank,province,city];
    ...
    if (isSearch == NO) {
        ...
        if ([_searchArray[1] isEqualToString:@"医生"]) {
            [contactsBll getSearchDoctorsResult:^(NSArray *arr) {
                ...
                [_slimeView endRefresh];
                [_tableView footerEndRefreshing];
            } pageIndex:pageIndex keywords:_searchArray[0] kind:_searchArray[1] department:_searchArray[2] viewCtrl:self];
        }else {
            [contactsBll getSearchHospitalsResult:^(NSArray *arr) {
                ...
                [_slimeView endRefresh];
                [_tableView footerEndRefreshing];
            } pageIndex:pageIndex keywords:_searchArray[0] kind:_searchArray[1] hospitalrank:_searchArray[3] province:_searchArray[4] city:_searchArray[5] viewCtrl:self];
        }
    }
...
//    return cell;
    ...
    if ([_searchArray[1] isEqualToString:@"医生"]) {
        ...
        if (!cell2) {
            ...
            if (_type == 6) {
                [cell2 selectButtonAction:^(ContactsViewCell *selectCell) {
        ...
        ContactModel *model = dataArray[indexPath.row];
        if (_type == 6) {
            [cell2 fillData:model hideSelectBtn:NO];
        }else {
            [cell2 fillData:model];
        }
        return cell2;
    }
}
...
#pragma mark 上拉加载
-(void)createMJ{
    __block  ContactsViewController *contactsVC = self;
