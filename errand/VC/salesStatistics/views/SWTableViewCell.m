...
#define kUtilityButtonHeightDefault self.frame.size.height/2
 ...
 *  初始化数组中的 btn
 *
 *  @param utilityButtons        btn数组
 *  @param parentCell            父视图 cell
 *  @param utilityButtonSelector btn 点击方法
 *
 ...
 *  @param utilityButtons        <#utilityButtons description#>
 *  @param parentCell            <#parentCell description#>
 *  @param utilityButtonSelector <#utilityButtonSelector description#>
 *
 *  @return <#return value description#>
 */
...
// WJJ 添加
- (CGFloat)calculateUtilityButtonHeight {
    CGFloat buttonHeight = kUtilityButtonHeightDefault;
    return buttonHeight;
}
...
//    cellScrollView.backgroundColor = [UIColor lightGrayColor];
    ...
    SWUtilityButtonView *scrollViewButtonViewRight = [[SWUtilityButtonView alloc] initWithUtilityButtons:_rightUtilityButtons parentCell:self utilityButtonSelector:nil];
...
- (void)layoutSubviews {
    ...
    self.scrollViewButtonViewLeft.frame = CGRectMake(0, 0, 0, _height);
    ...
    self.scrollViewContentView.backgroundColor = [UIColor yellowColor];
...
//    return ([_scrollViewButtonViewLeft utilityButtonsWidth] + [_scrollViewButtonViewRight utilityButtonsWidth]);
    return _scrollViewButtonViewRight.utilityButtonWidth;
}
...
- (void)scrollViewDidScroll:(UIScrollView *)scrollView {
    if (scrollView.contentOffset.x > [self leftUtilityButtonsWidth]) {
        ...
        self.scrollViewButtonViewRight.backgroundColor = [UIColor clearColor];
        NSInteger width = self.scrollViewButtonViewRight.frame.size.width;
        NSInteger height = self.scrollViewButtonViewRight.frame.size.height;
        
        // 删除 btn
        UIButton *deleteBtn = [[UIButton alloc] initWithFrame:CGRectMake((width - height/2 + 20) / 2, 10, height / 2 - 20, height / 2 - 20)];
        deleteBtn.backgroundColor = [UIColor blueColor];
        deleteBtn.tag = 1;
        deleteBtn.layer.cornerRadius = (height / 2 - 20)/2;
        deleteBtn.layer.masksToBounds = YES;
        [deleteBtn addTarget:self action:@selector(deleteAndEditBtnClick:) forControlEvents:UIControlEventTouchUpInside];
        [self.scrollViewButtonViewRight addSubview:deleteBtn];
        
        // 编辑 btn
        UIButton *eidtBtn = [[UIButton alloc] initWithFrame:CGRectMake((width - height/2 + 20) / 2, 10 + height/2, height / 2 - 20, height / 2 - 20)];
        eidtBtn.backgroundColor = [UIColor blueColor];
        eidtBtn.tag = 2;
        eidtBtn.layer.cornerRadius = (height / 2 - 20)/2;
        eidtBtn.layer.masksToBounds = YES;
        [eidtBtn addTarget:self action:@selector(deleteAndEditBtnClick:) forControlEvents:UIControlEventTouchUpInside];
        [self.scrollViewButtonViewRight addSubview:eidtBtn];
    } else {
        // Expose the left button view
        self.scrollViewButtonViewLeft.frame = CGRectMake(scrollView.contentOffset.x, 0.0f, [self leftUtilityButtonsWidth], _height);
    }
}
// 两个 btn 的点击方法, 该方法已暴露在 .h 中，可被子类继承
- (void)deleteAndEditBtnClick:(UIButton *)btn {
    if (btn.tag == 1) {
        NSLog(@"删除");
    } else {
        NSLog(@"编辑");
